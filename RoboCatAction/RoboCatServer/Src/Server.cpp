...
Server::Server()
{
	...
	GameObjectRegistry::sInstance->RegisterCreationFunction('SHLD', ShieldServer::StaticCreate);
...
namespace
{
	...
	void CreateRandomMice( int inMouseCount )
	{
		...
		for( int i = 0; i < inMouseCount - 1; ++i )
		{
	...
	void CreateRandomShield(int inShieldCount)
	{
		Vector3 shieldMin(-5.f, -3.f, 0.f);
		Vector3 shieldMax(5.f, 3.f, 0.f);
		GameObjectPtr go;
		go = GameObjectRegistry::sInstance->CreateGameObject('SHLD');
		Vector3 shieldLocation = RoboMath::GetRandomVector(shieldMin, shieldMax);
		go->SetLocation(shieldLocation);
	}
}
